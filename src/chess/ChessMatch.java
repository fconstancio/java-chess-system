...
public class ChessMatch {
	...
	private boolean[][] oponentsPossibleMoves = new boolean[8][8];
	...
	public boolean[][] getOppenentPossibleMoves() {
		return oponentsPossibleMoves;
	}

	public void updateOpenentPossibleMove() {
		oponentsPossibleMoves = calculatorOponentsPossibleMoves(currentPlayer);
	}
	...
	private void placeNewPiece(char chessColumn, int chessRow, ChessPiece chessPiece) {
		board.placePiece(chessPiece, new ChessPosition(chessColumn, chessRow).toPosition());
...
//		LogUtil.fromPerformChessMove(board.piece(target), capturedPiece, isCheck(), isCheckMate(), turn);
	...
	private boolean testCheck(Color color) {
		...
		List<Piece> oppenentPieces = piecesOnTheBoard.stream()
				.filter(p -> ((ChessPiece) p).getColor() == oponent(color)).collect(Collectors.toList());
		for (Piece piece : oppenentPieces) {
	...
	private boolean[][] calculatorOponentsPossibleMoves(Color color) {
		boolean[][] opm = new boolean[board.getRows()][board.getColumns()];
//		Arrays.stream(opm).forEach(a -> Arrays.fill(a, Boolean.FALSE));
		List<Piece> oppenentPieces = piecesOnTheBoard.stream()
				.filter(p -> ((ChessPiece) p).getColor() == oponent(color)).collect(Collectors.toList());
		for (Piece piece : oppenentPieces) {
			boolean[][] matPiece = piece.possibleMoves();
			for (int i = 0; i < opm.length; i++) {
				for (int j = 0; j < opm[i].length; j++) {
					opm[i][j] = opm[i][j] || matPiece[i][j];
				}
			}
		}
//		System.out.println("- OponentsPossibleMoves -\n");
//		for (int i = 0; i < opm.length; i++) {
//			System.out.print(i + " ");
//			for (int j = 0; j < opm[i].length; j++) {
//				System.out.print((opm[i][j]?"V":"F") + " ");
//			}
//			System.out.println();
//		}
